...
class EventLogDiCEOptimized:
    ...
    def _init_cat_logits(
        self,
        cat_arrs: List[np.ndarray],
        cat_dims: List[int],
        initial_logit_scale: float = 3.0,
    ) -> List[torch.Tensor]:
        """
        Initialize learnable categorical logits from encoded category indices.
    
        The factual category receives a larger initial logit so the initial
        Gumbel-Softmax output is close to the factual one-hot category.
        """
        ...
        for arr, dim in zip(cat_arrs, cat_dims):
            ...
            logits = one_hot * initial_logit_scale
            logits_list.append(logits.detach())
    
        return logits_list

    def _categorical_outputs_from_logits(
        self,
        cat_logits:   List[torch.Tensor],
        initial_ohe:  List[torch.Tensor],
        ...
        for feature_idx, (logits, original) in enumerate(
            zip(cat_logits, initial_ohe)
        ):
            update_mask = output_masks.get(feature_idx)
    
            # Entire feature immutable.
            ...
            # Preserve forbidden positions exactly.
            outputs.append(
                torch.where(
                    update_mask.bool(),
                    sampled,
    ...
    def _project_mutable_cat_logits(
        self,
        cat_logits:    List[torch.Tensor],
        initial_ohe:   List[torch.Tensor],
        output_masks:  Dict[int, torch.Tensor],
        ...
        for feature_idx, (logits, original) in enumerate(
            ...
            candidate = self._project_cat_logits(
                cat_logits=[logits],
    ...
    def _project_cat_logits(
        self,
        cat_logits:    List[torch.Tensor],
        use_sampling:  bool = False,
    ) -> List[torch.Tensor]:
        """
        Convert categorical logits into valid one-hot tensors.
    
        use_sampling=False:
            Select the category with the largest logit.
    
        use_sampling=True:
            Sample from the categorical distribution defined by softmax(logits).
        """
    ...
    def _collect_cat_optimizer_vars(
        ...
        # feature_idx -> mask with the same shape as that categorical tensor
        output_masks: Dict[int, torch.Tensor] = {}
    
        vary_features = set(split_vary_idx.tolist())
    
        for feature_idx, tensor in enumerate(cat_logits):
    
            original = tensor.detach().clone()
    
            # Entire feature is immutable.
            ...
            # Static categorical: (B, K)
            if variable.ndim == 2:
                update_mask = torch.ones_like(variable)
    
            # Dynamic categorical: (B, T, K)
            ...
            # Feature is nominally mutable, but no entries can change.
    ...
    def _calculate_optimization_losses(
        ...
        cat_loss = margin_loss.new_tensor(0.0) # gumbel-softmax gurantees
    ...
    def _sparsity_loss(
        ...
        static_cont_update_mask:  Optional[torch.Tensor],
        mask:                     Optional[torch.Tensor] = None,
        continuous_temperature:   float = 0.01,
    ) -> torch.Tensor:
        
        """
        Differentiable approximation of the proportion of mutable cells changed.
    
        Categorical cells:
            0.5 * L1 distance between one-hot vectors.
            For hard one-hot values:
                unchanged -> 0
                changed   -> 1
    
        Continuous cells:
            1 - exp(-|x' - x| / temperature)
            This is:
                exactly 0 when unchanged;
                approaches 1 as the change becomes larger.
    
        Only mutable cells are included in the denominator.
        Padded dynamic positions are excluded.
        """
    
        # Find a reference tensor for device and dtype.
        if dyn_cat_ohe:
            reference = dyn_cat_ohe[0]
        elif static_cat_ohe:
            reference = static_cat_ohe[0]
        elif dyn_cont.numel() > 0:
            reference = dyn_cont
        elif static_cont.numel() > 0:
            reference = static_cont
        else:
            raise ValueError("No feature tensors were provided.")
    
        changed_sum = reference.new_tensor(0.0)
        ...
        for feature_idx, (current, original) in enumerate(
            ...
            if current.shape != original.shape:
                raise ValueError(
                    "Dynamic categorical current/original shape mismatch: "
                    f"{current.shape} != {original.shape}."
                )
    
            if update_mask.shape != current.shape:
                raise ValueError(
                    "Dynamic categorical update-mask shape mismatch: "
                    f"{update_mask.shape} != {current.shape}."
                )
    
            # One score per categorical feature cell: (B, T)
            change_score = (
                current - original
            ).abs().sum(dim=-1) / 2.0
    
            # Reduce category-level mask (B, T, K) to cell mask (B, T).
            valid = update_mask.bool().any(dim=-1).to(
                device=current.device,
                dtype=current.dtype,
            )
    
            if mask is not None:
                if mask.shape != change_score.shape:
                    raise ValueError(
                        "Sequence mask must have shape (B, T): "
                        f"{mask.shape} != {change_score.shape}."
                    )
        ...
        for feature_idx, (current, original) in enumerate(
            ...
            if current.shape != original.shape:
                raise ValueError(
                    "Static categorical current/original shape mismatch: "
                    f"{current.shape} != {original.shape}."
                )
    
            if update_mask.shape != current.shape:
                raise ValueError(
                    "Static categorical update-mask shape mismatch: "
                    f"{update_mask.shape} != {current.shape}."
                )
        ...
        if (
            dyn_cont.numel() > 0
            and dyn_cont_update_mask is not None
        ):
            if dyn_cont.shape != dyn_cont_0.shape:
                raise ValueError(
                    "Dynamic continuous current/original shape mismatch: "
                    f"{dyn_cont.shape} != {dyn_cont_0.shape}."
                )
    
            if dyn_cont_update_mask.shape != dyn_cont.shape:
                raise ValueError(
                    "Dynamic continuous update-mask shape mismatch: "
                    f"{dyn_cont_update_mask.shape} != {dyn_cont.shape}."
                )
    
            valid = dyn_cont_update_mask.to(
                device=dyn_cont.device,
                dtype=dyn_cont.dtype,
            )
    
            if mask is not None:
                if mask.shape != dyn_cont.shape[:2]:
                    raise ValueError(
                        "Sequence mask must have shape (B, T): "
                        f"{mask.shape} != {dyn_cont.shape[:2]}."
                    )
            ...
            change_score = 1.0 - torch.exp(
                -absolute_difference
                / max(continuous_temperature, 1e-8)
            )
        ...
        if (
            static_cont.numel() > 0
            and static_cont_update_mask is not None
        ):
            if static_cont.shape != static_cont_0.shape:
                raise ValueError(
                    "Static continuous current/original shape mismatch: "
                    f"{static_cont.shape} != {static_cont_0.shape}."
                )
    
            if static_cont_update_mask.shape != static_cont.shape:
                raise ValueError(
                    "Static continuous update-mask shape mismatch: "
                    f"{static_cont_update_mask.shape} != "
                    f"{static_cont.shape}."
                )
    
            valid = static_cont_update_mask.to(
        ...
        return changed_sum / valid_count.clamp_min(1e-8)
    ...
    def _run(
        ...
        dyn_cat_logits = self._init_cat_logits(
            model_inputs["dynamic_cat"], dynamic_cat_dims, self.dice4el_config.initial_logit_scale
        )
        static_cat_logits = self._init_cat_logits(
            model_inputs["static_cat"], static_cat_dims, self.dice4el_config.initial_logit_scale
        )

        dyn_cat_initial_ohe = self._init_cat_ohe(
            model_inputs["dynamic_cat"], dynamic_cat_dims
        )
        
        static_cat_initial_ohe = self._init_cat_ohe(
        ...
        (dyn_cat_logits, dyn_cat_optimizer_vars, dyn_cat_grad_masks, dyn_cat_update_masks) = self._collect_cat_optimizer_vars(
            cat_logits=dyn_cat_logits,
            split_vary_idx=split_vary_indices["split_vary_dynamic_cat_idx"],
            restricted_positions=restricted_positions,
        )
        
        (static_cat_logits, static_cat_optimizer_vars, static_cat_grad_masks, static_cat_update_masks) = self._collect_cat_optimizer_vars(
            cat_logits=static_cat_logits,
        ...
        for step in range(self.dice4el_config.optimization_steps):
            ...
            tau = max(
                self.dice4el_config.gumbel_min_temperature,
                self.dice4el_config.gumbel_initial_temperature
                * self.dice4el_config.gumbel_anneal_rate ** step,
            )
            
            dyn_cat_ohe = self._categorical_outputs_from_logits(
                cat_logits=dyn_cat_logits,
                initial_ohe=dyn_cat_initial_ohe,
                output_masks=dyn_cat_update_masks,
                tau=tau,
                hard=self.dice4el_config.gumbel_hard,
            )
            
            static_cat_ohe = self._categorical_outputs_from_logits(
                cat_logits=static_cat_logits,
                initial_ohe=static_cat_initial_ohe,
                output_masks=static_cat_update_masks,
                tau=tau,
                hard=self.dice4el_config.gumbel_hard,
            )
            ...
            with torch.no_grad():
                projected_dyn_cat_ohe = (
                    self._project_mutable_cat_logits(
                        cat_logits=dyn_cat_logits,
                        initial_ohe=dyn_cat_initial_ohe,
                        output_masks=dyn_cat_update_masks,
                        use_sampling=False,
        ...
        final_dyn_cat = self._valid_cat_from_logits(best_projected_state["dynamic_cat"])
        final_static_cat = self._valid_cat_from_logits(best_projected_state["static_cat"])
